...
import { useCreateBookingMutation, useGetBookingStatsQuery, useGetBookingsQuery, useUpdateBookingStatusMutation } from '../bookings/bookingsApi'
...
export function ShipmentsPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-4">
      ...
      <Modal open={open} wide title="Create New Shipment" onClose={() => setOpen(false)}>
              ...
              <div className="space-y-2">
                {staff.map((s) => (
                  <label key={s._id} className="flex items-center justify-between rounded-xl bg-white px-3 py-2 text-sm">
                    ...
                    <span className="text-emerald-700">{formatMoney(s.incentiveRate)}/quintal</span>
                  </label>
                ))}
              </div>
            </div>
        ...
        <div className="mt-6 flex justify-between">
          ...
          {step < 4 ? <Button onClick={() => setStep((s) => s + 1)}>Next</Button> : <Button loading={creating} onClick={submit}>Create Shipment</Button>}
        </div>
      </Modal>
    </div>
  )
