...
import { createSettlementSchema, settleSchema, recordAdvanceSchema } from './settlement.validation.js';

const router = Router();
router.use(authenticate);

router.get('/', requirePermission('expenses:view'), ctrl.listSettlements);
router.get('/board', requirePermission('expenses:view'), ctrl.tripSettlementBoard);
...
router.post('/advance', requirePermission('expenses:approve'), validate(recordAdvanceSchema), ctrl.recordAdvance);
router.post('/trip/:tripId/settle', requirePermission('expenses:approve'), ctrl.settleTrip);
