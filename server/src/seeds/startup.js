...
async function seedRoles(allPermissions) {
  const byCode = Object.fromEntries(
    allPermissions.map((p) => [p.code, p._id])
  );
...
async function seedSettings() {
  const defaults = [
    {
      key: 'company.name',
      value: 'SwiftHaul Transport',
      group: 'company',
      description: 'Company display name',
    },
    {
      key: 'company.tagline',
      value: 'Transport Management ERP',
      group: 'company',
    ...
    },
    {
      ...
      value: '',
      group: 'company',
      description: 'Dashboard logo URL',
    },
    {
      key: 'company.faviconUrl',
      value: '',
      group: 'company',
    ...
    },
    {
      ...
      value: 'office@tms.local',
      group: 'company',
      description: 'Public support email',
    },
    {
      key: 'company.supportPhone',
      value: '+91-9999999999',
      group: 'company',
    ...
    },
    {
      ...
      value: 'BK',
      group: 'bookings',
      description: 'Booking number prefix',
    },
    {
      key: 'trip.prefix',
      value: 'TR',
      group: 'trips',
      description: 'Trip number prefix',
    },
    {
      key: 'invoice.prefix',
      value: 'INV',
...
export async function seedInitialData() {
  console.log('[startup] Checking initial database data...');

  const permissions = await seedPermissions();
  console.log(`[startup] Permissions ready: ${permissions.length}`);

  await seedRoles(permissions);
  console.log('[startup] Roles ready');

  const branch = await seedBranch();
  console.log('[startup] Head Office ready');

  await seedSettings();
  console.log('[startup] Settings ready');

  const admin = await seedAdmin(branch);
  console.log(`[startup] Admin ready: ${admin.email}`);

  // Demo operational data only when there are no customers
  await seedDemoIfEmpty();

  console.log('[startup] Initial database setup complete');
}
